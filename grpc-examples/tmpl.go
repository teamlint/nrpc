...
const tFile = `// Code generated by protoc-gen-nrpc from {{.GetName}}. DO NOT EDIT.
...
func (h *{{.GetName}}Handler) MsgHandler(msg *nats.Msg) {
...
// range Method begin
{{- range .Method}}
// resultType cond begin
{{- $resultType := GetResultType .}}
// nprc.NoRequest begin
{{- if eq .GetInputType ".nrpc.NoRequest"}}
...
func (c *{{$serviceName}}Client) {{.GetName}}(
	...
	if err != nil {
		{{- if Prometheus}}
		clientCallsFor{{$serviceName}}.WithLabelValues("{{.GetName}}", c.Encoding, "call_fail").Inc()
		{{- end}}
	...
	elapsed := time.Since(start).Seconds()
	clientRCTFor{{$serviceName}}.WithLabelValues("{{.GetName}}").Observe(elapsed)
	clientCallsFor{{$serviceName}}.WithLabelValues("{{.GetName}}", c.Encoding, "success").Inc()
	{{- end}}
